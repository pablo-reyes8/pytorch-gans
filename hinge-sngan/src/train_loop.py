...
def train_gan(train_loader, generador, discriminador,
    ...
    latent_dim = 100, epochs = 20, gamma = 10, # Hiperparammetro para R1 pennalty
    ...
    Entrena una GAN : BCE+logits o Hinge (SNGAN) con R1 opcional.
      - disc_steps: D se actualiza varias veces por batch (opcional)
      ...
      - Tambien soporta entrenamiento no Hinge y CUDA
    """
    ...
    if use_ema:
        ...
        for p in gen_ema.parameters():
            p.requires_grad_(False) # Solo se usa para inferencia
    ...
    if fixed_z is None:
        ...
        with torch.no_grad():
            fake0 = (gen_ema if use_ema else generador)(fixed_z).detach() # Muestreamos con EMA no con generador
    ...
    for epoch in range(1, epochs + 1):
        ...
        # warm-up de G: más pasos de G por batch durante X épocas 
        ...
        for real, _ in train_loader:
            ...
            for _ in range(disc_steps):
                ...
                if hinge:
                    ...
                    if need_r1:
                        real_for_r1 = real.detach().requires_grad_(True) # Pedimos gradientes para las imagenes en el grafo
                        real_in = real_for_r1
                    else:
                        real_for_r1 = real
                        real_in = real
                ...
                # falsas para D
                z = torch.randn(b, latent_dim, device=device) # Generaos falas imagenes
                fake = generador(z).detach() # Quitamos el generador del grafo para no actualizar sus pesos
                ...
                if hinge:
                    ...
                    if need_r1: # Aplicamos pennalty R1 si es necesario 
                        r1 = r1_penalty(out_real, real_for_r1)
                        loss_D = loss_D_core + 0.5 * gamma * r1
                    else:
                        loss_D = loss_D_core
                else: 
                    # Si no se usa Hinge usamos perdida BCE normal
                    if smooth:
            ...
            for _ in range(g_steps_this_epoch):
                ...
                out_fake_for_G = discriminador(fake_in_for_G) # Aca si actualizamos los pesos del G 

                if hinge: # Perdida Hinge 
                    loss_G = loss_hinge_generator(out_fake_for_G)
                else:
                    # Perdida no-saturante con BCE
                    if smooth:
                        real_labels = torch.full((b, 1), 0.9, device=device)
                    elif smooth_advance:
                        real_labels = torch.empty(b, 1, device=device).uniform_(0.8, 1.0)
                ...
                # actualizar EMA del generador tras cada paso de G 
                if use_ema:
